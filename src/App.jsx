...
const buildLink = (name) => `${INVITATION_URL}?guest=${buildGuestParam(name)};`;

// Versi Pengantin: dikirim oleh Vira & Ogi sendiri
const buildCoupleLetter = (name) => `Bismillahirrahmanirrahim
...
const buildParentsLetter = (name) => `Assalamualaikum Warahmatullahi Wabarakatuh
...
const App = () => {
  ...
  const handleCopy = (version) => {
    if (!isReady) return;
    copyToClipboard(version.build(trimmedName));
    Swal.fire({
      icon: "success",
      title: "Berhasil disalin 😀",
      html: `Undangan <b>${version.title}</b> untuk <b>${capitalizeWords(
        trimmedName
      )}</b> sudah ada di clipboard.<br/>Tinggal tempel (paste) di WhatsApp.`,
    });
  };
        ...
        <section className="rounded-lg bg-[#2b2f2f] p-4">
          ...
          <div className="mt-3 text-xs">
            ...
            <div className="mt-1 break-all rounded bg-[#1f2222] px-2 py-1 font-mono text-[#e0c9cc]">
              {isReady ? buildLink(trimmedName) : `${INVITATION_URL}?guest=...`}
            </div>
          </div>
        </section>
        ...
        <section className="flex flex-col gap-4">
          ...
          {!isReady && (
            <p className="text-center text-xs text-[#b0b0b0]">
              Isi nama tamu terlebih dahulu untuk mengaktifkan tombol salin.
            </p>
          )}
        </section>
        ...
        >
          ...
          >
            ...
            <pre className="flex-1 overflow-auto whitespace-pre-wrap px-4 py-3 text-sm leading-relaxed">
              {preview.build(isReady ? trimmedName : "Nama Tamu")}
            </pre>
